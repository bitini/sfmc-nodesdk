...
module.exports = {
	...
	getToken: async function () {
		...
		let payload = {
			...
			"redirect_uri": this.config.rediret_uri
		};
	...
	getUserInfo: async function (token) {
		...
		return new Promise((resolve, reject) => {
			fetch(
				`${this.config.authBaseUrl}v2/userinfo`, {
					method: 'GET',
			...
			.then(res => res.json())
			.then(userinfo => { userinfo = [userinfo.organization.member_id, userinfo.organization.stack_key];
								resolve(userinfo);
				})
				.catch(error => reject(error));
		});
